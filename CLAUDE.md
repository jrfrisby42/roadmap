...
**CLASSIC IS UNLINKED AND UNSUPPORTED (policy, CLASSIC-SUNSET-1).** The "Switch to classic UI" menu entry was removed deliberately - the Flow shell is the standard UI, and classic (`?classic=1`) is reachable only by typing the query string or an old bookmark. Read this before touching shared markup:
1. **New work does not need to accommodate classic.** If a shared component (the List, the Gantt, the item page) styles or behaves correctly only in the shell, that is acceptable - degrade classic rather than engineering around it. Two recent stages paid this tax needlessly: LIST-SCROLL-1 could not use `--frz-*` tokens on the List scrollbar proxy (shared markup, tokens don't resolve outside `.frz-beta`), and SCROLL-CARET-1 had to unscope `.frz-chip-caret` + add a `var(--frz-text-muted, var(--muted))` fallback because 5 of 7 caret sites were classic-reachable. Both were correct, both existed only because classic does. Stop doing that.
2. **Do not deliberately break it.** It is the fallback if the shell fails to mount (classic is script block B, the shell is block C - a parse error isolated to the shell block would spare classic; a `boot()` runtime failure may not). Incidental degradation from shell-first work is fine; going out of your way to break classic is not.
3. **It is not verified on deploy.** Assume it may already be degraded in ways nobody has noticed (health baseline lives in the CLASSIC-SUNSET-1 deliverable / this session's report).
4. **Removal is a future dedicated stage, not a drive-by.** Removing it means unscoping the classic-only subset of the `.frz-beta` rules (NOT all of them - most `.frz-beta` scoping also isolates shell styles from the reused production views WITHIN the shell, and survives classic's removal) and untangling the `#frzContent` re-parent coupling. Do not begin it unprompted.
...
- "Items" and "projects" are the same thing in this codebase — historical naming. The actual project/product (Fraznet, HubSpot, etc.) is a separate config key called `products`, **displayed in the UI as "Space"** (TERM-1 renamed the label "Project" → "Space"; the stored field/key stay `product`/`products`). The tenant (config/DB `team`, `X-Team`) is **displayed as "Organization"** (TERM-1); the identifier stays `team`. The grouping field (blob `dev`, config `developers`, DB column `owner`, saved-view key `devs`, user field `ownerFilter`) is **displayed as "Team"** (TERM-3 renamed the label "Owner" → "Team"; every identifier stays). So "Team" the label is a different thing from "Organization" the tenant and from `team` the identifier - three distinct concepts sharing the string.
